...
public class ApiResource {
    ...
    // bookID (not ID; not book_id;)
    // always include description, rating, isFeatured
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            if (category == null) {
                throw new ApiException(String.format("No such books bu category name: %s", categoryName));
            }
            return bookDao.findByCategoryId(category.categoryId());
        } catch (Exception e) {
            throw new ApiException(String.format("Category lookup by category-name %s failed", categoryName), e);
    ...
    public List<Book> suggestedBooks(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            throw new ApiException("suggested products lookup via categoryName failed", e);
        }
    }
